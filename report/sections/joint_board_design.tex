...
\end{figure}
\subsubsection{Battery Type}
\label{subsub:battery}
\mikkel{This stuff is apparently also described in the analysis}
...
Furthermore needs to have enough capacity to power the circuit for a full workday of a student, which is, based on the authours experience, estimated to be 10 hours.
...
Lithium-Ion batteries have a very high energy density and it should therefore be possible to find a battery that fits both the physical and capacity constraints.
It was decided to use a Li-Ion battery for the joint. 
Before choosing a battery the power needed to run the electronics should be analysed.
...
\subsubsection{3.3V}
A 3.3V rail is needed according to section \ref{subs:joint_requirements}.
