...
\thomas{update figure \ref{fig:mosfettransfercharacteristic} to new fet, datasheet cite and values}
\begin{figure}[H]
	\centering
...
\begin{figure}[h]
	...
	\caption[.......]{REMEMBER TO PUT IN A COUPLE OF REFERENCES TO THIS FIGURE!!!}
	\label{fig:hip_bootstrap}
\end{figure}	
\thomas{insert references to figure \ref{fig:hip_bootstrap}}
...
\mikkel{make sure 95\%/5\% is correct.}
...
The circuitry is identical for the main relay but can be seen in the full schematic in appendix \ref{}.
\mikkel{add schematics to appendix.}
...
\mikkel{What can we achieve with the hardware?}
...
It was decided to use the \texttt{TCST2103} \cite{} as it has the wanted functionality.
\thomas{Cite me!}
...
The circuit can be seen in appendix \ref{}
\thomas{reference to appendix schematics}
...
\end{figure}
\thomas{Create correct circuit and text}
...
\thomas{fix reference to hardware user guide and change sentence to explain 3.3 vs 5V distribution.}
...
The PTH08080 design will be reused for this project.
% paragraph 5 (end)
...
The first, the G6B, requires 16.7mA while the former, the LEV100A4ANG, requires 461mA.
