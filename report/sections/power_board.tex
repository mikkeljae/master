...
Calculations and simulation in PLECS is used to determine the size and requirements of the load capacitors.

The ripple current across the motor inductor needs to be determined as this is the current the supply capacitors need to supply.
...
The peak-to-peak ripple current of $6.7$A is also found when simulation the system using PLECS as shown in figure \ref{fig:sim_currents}.
...
The minimum capacitor can be calculated by refactoring the general capacitor equation shown in \ref{eq:cap_gen} into equation 
\begin{equation} 
...
\begin{equation} 
	...
	\label{eq:cap_min}
\end{equation}
There is no specified requirements for the maximum allowable voltage ripple on such a board, but it is generally undesired and it was decided that the board should be designed should that it it less than 1V.
...
Connecting two of these capacitors in parallel would yield a total allowed ripple current of 2.6A, which would be enough.
To allow for some headroom it was chosen to use four which yields a combined maximum ripple current of 5.3A. 
...
Therefore it was chosen to add ceramic capacitor as they perform much better at high frequencies. 
...
Based on previous experience it was decided, to use four 1$\mu$F ceramic capacitors in the \texttt{0805} housing as they can easily be changed if it turns more capacitance is needed.
...
It can also be noted that an effect of the inductance in the supply wires and the supply capacitors is that the supply current is low pass filtered and is thereby protected against almost all of the current ripple and only needs to supply the DC component of the load current.
