...
\thomas{done - section introduction}
Throughout this section the software for the controller board is developed.
An analysis is made to determine the required functionality of the software.
An overview of the computing platform and the utilization of it in this project is also given.
Software is implemented to test the hardware. 
Finally a verification is done of the controller board software requirements.
...
The current should be sampled at the center of a PWM period, as switching introduces noise that should not be measured.
This is easily done by implementing center aligned PWM and sampling at the top signal.
\mikkel{done - SQUIRREL!}
...
\subsection{Requirement Specification} % (fold)
...
The \texttt{clk\_proc} can be seen in listing \ref{code:clk_proc}.
The remaining processes are similar in function but operate on different signals.
...
\end{minted}
\caption[Process generating the clock signal in the test bench]{\texttt{clk\_proc} is the process responsible for generating the clock signal in the test bench.}
\label{code:clk_proc}
\end{listing}

\thomas{done - added code for testbench}
...
The GPIO block is used to signal an unmet safety condition from software, enabling the motor driver and similar tasks.
\thomas{done - rewrite for proper safety condition}
...
This requirement specifies that software should be able to handle safety condition.
\thomas{done - do safety condition transformation}
\paragraph{Test}~\\
The \texttt{EM\_DIS} signal should be driven high by moving the cart to one of the endstops.
This indicates an unmet safety condition and the \texttt{EM\_DIS} should be recorded using the MicroZed.
\\
The \texttt{EM\_MCU} signal should be driven \texttt{low} to ensure that this causes an unmet safety conditions.
