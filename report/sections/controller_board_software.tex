...
\missingfigure{Put in a nice illustration}
...
\paragraph{The AXI Bus} % (fold)
\label{par:the_axi_bus}

% paragraph the_axi_bus (end)
...
\begin{enumerate}[resume]
	...
	\item C or VHDL...??????????
\end{enumerate}
\mikkel{Write something clever about C or VHDL!}
\paragraph{Safety:}
\begin{enumerate}[resume]
...
\\~\\
\paragraph{Incremental Quadrature}~\\ % (fold)
\label{par:incremental_quadrature}
As mentioned above, the motor provides three signals for incremental quadrature, \texttt{A}, \texttt{B} and \texttt{Z}. 
...
\end{figure}
% paragraph incremental_quadrature (end)
...
A depiction of the state machine (SM) can be seen in figure \ref{fig:quadstatemachine}.
Note that VHDL notation has been omitted for clarity.
As can be seen, the SM is comprised of six states.
...
\paragraph{st4\_cw:} % (fold)
\paragraph{st5\_ccw:} % (fold)
\paragraph{st6\_index:} % (fold)

\begin{itemize}
	\item \textbf{st1\_idle:} This 
	
	\item \textbf{st2\_edge:} 
	\item \textbf{st3\_dir:} 
	\item \textbf{st4\_cw:} 
	\item \textbf{st5\_ccw:} 
	\item \textbf{st6\_index:} 
\end{itemize}
