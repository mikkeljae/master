...
This requirement specifies that the \texttt{nRF24L01} module should be included for communication.

\paragraph{Conclusion}~\\
The \texttt{nRF24L01} was included in the joint board PCB and wired to the \texttt{ATtiny84}.
...
This requirement specifies that the design should allow programming of the \texttt{ATtiny84} using 5V SPI.

\paragraph{Conclusion}~\\
Programming the \texttt{ATtiny84} using 5V SPI was made possible using voltage level shifting.
While developing software for the \texttt{ATtiny84} it was revealed that the \texttt{74LCX244} and \texttt{ATtiny84} both attempt to drive the \texttt{SCLK} and \texttt{MOSI} nets, resulting in the communication with the nRF24L01 module not functioning.
...
In running mode the \texttt{74LCX244} is disabled and the \texttt{ATtiny84} is capable of driving the affected nets.
