...
The circuit can be seen in \ref{}
\mikkel{Find appropriate circuit}
...
\begin{itemize}
 	...
 	\end{itemize}
 \end{itemize}
\mikkel{Move microzed startup bla to requirement section, refactor for new format}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item[-] The important signals when shutting down the controllerboard and the Microzed are measured and plotted in figure \ref{fig:controllerboardv2_shutdown}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\mikkel{insert dead time figure}
	\end{itemize}
\end{itemize}
...
\mikkel{This should be "flettet" sammen.}
The voltages of the rails were measured for 20ms and the mean voltage on each rail is shown in table \ref{tab:meas_rails}.
\begin{table}[]
\centering
...
Connecting the Microzed to the controller board PCB is done through two Bergstak 100pins headers, where all appropriate signals are routed to.
...
\mikkel{Insert conclusion, remove number}
...
\subsubsection{Verification of: Requirement \ref{enum:not_break_mis}} % (fold)
