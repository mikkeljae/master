...
void RF_write_address(uint8_t reg, uint8_t *addr){
  uint8_t output_buffer[]  = {(W_REGISTER | ( REGISTER_MASK & reg)), addr[0], addr[1], addr[2], addr[3], addr[4]};
  spi_transfer(output_buffer, NULL, 6);  //Send 6 bytes. Receive nothing
}
...
void RF_begin(void){
  ...
  RF_write_register(RF_CONFIG, 0x0C);

  //Read setup register
  uint8_t setup;
  RF_read_register(RF_SETUP, &setup);
...
void RF_open_reading_pipe(void){
  uint8_t rf_address[] = {RF_ADDRESS0, RF_ADDRESS1, RF_ADDRESS2, RF_ADDRESS3, RF_ADDRESS4};
...
void RF_set_TX_power(void){
  uint8_t setup;
  RF_read_register(RF_SETUP, &setup);
  RF_write_register(RF_SETUP, setup | RF_MAX_OUT_P);
}
