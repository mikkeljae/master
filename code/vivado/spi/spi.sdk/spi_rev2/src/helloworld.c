...
#define RF_ADDRESS2		0b00110000
#define RF_ADDRESS3		0b00110000
#define RF_ADDRESS4		0b00110001
...
int main()
{
	...
	while(1){
			...
			//RF_write_register(&SPI, RF_CH, 76);
...
void RF_begin(XSpiPs *SPI_inst){
	// Reset config register and set 16-bit CRC
...
void RF_open_reading_pipe(XSpiPs *SPI_inst){
	u8 rf_address[] = {RF_ADDRESS0, RF_ADDRESS1, RF_ADDRESS2, RF_ADDRESS3, RF_ADDRESS4};
...
void RF_write_address(XSpiPs *SPI_inst, u8 reg, u8 *addr){
	u8 output_buffer[]  = {(W_REGISTER | ( REGISTER_MASK & reg)), addr[0], addr[1], addr[2], addr[3], addr[4]};
	XSpiPs_PolledTransfer(SPI_inst, output_buffer, NULL, 6); 	//Send 6 bytes. Receive nothing
}
